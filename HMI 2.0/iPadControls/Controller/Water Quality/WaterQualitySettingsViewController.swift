...
class WaterQualitySettingsViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var w1stpdose: UITextField!
    ...
    private func getSetpoints(){
        ...
        CENTRAL_SYSTEM?.readRegister(length: 9, startingRegister: Int32(WQ_PH_TIMER_REGISTER), completion:{ (success, response) in
            
            if success == true{
                
                let PHSetpoint  = Int(truncating: response![0] as! NSNumber)
                let ORPSetpoint = Int(truncating: response![4] as! NSNumber)
                let TDSSetpoint = Int(truncating: response![8] as! NSNumber)
        ...
        CENTRAL_SYSTEM?.readRegister(length: 5, startingRegister: Int32(WQ_BR_TIMER_REGISTER), completion:{ (success, response) in
            
            if success == true{
                let w1strtdseval = Int(truncating: response![0] as! NSNumber)
                let w1stopdseval = Int(truncating: response![1] as! NSNumber)
                let w1opnval = Int(truncating: response![2] as! NSNumber)
                let w1clsval = Int(truncating: response![3] as! NSNumber)
                let brominatorTimeout = Int(truncating: response![4] as! NSNumber)
                
                
                self.w1dose.text  = "\(w1strtdseval)"
                self.w1stpdose.text  = "\(w1stopdseval)"
                self.w1open.text  = "\(w1opnval)"
                self.w1close.text = "\(w1clsval)"
                self.brominatorTimeoutTxt.text = "\(brominatorTimeout)"
    ...
    private func saveSetpointDelaysToPLC(){
        ...
        let w1stopdoseVal  = Int(w1stpdose.text!)
        ...
        guard oprDelayTimerSP != nil && phDelayTimerSP != nil && tdsDelayTimerSP != nil && brominatorTimeoutSP != nil && w1openVal != nil && w1closeVal != nil && w1doseVal != nil && w1stopdoseVal != nil else { return }
        ...
        CENTRAL_SYSTEM?.writeRegister(register: WQ_BR_TIMER_REGISTER, value: w1doseVal!)
        CENTRAL_SYSTEM?.writeRegister(register: WQ_BR_TIMER_REGISTER+1, value: w1stopdoseVal!)
        CENTRAL_SYSTEM?.writeRegister(register: WQ_BR_TIMER_REGISTER+2, value: w1openVal!)
        CENTRAL_SYSTEM?.writeRegister(register: WQ_BR_TIMER_REGISTER+3, value: w1closeVal!)
        CENTRAL_SYSTEM?.writeRegister(register: WQ_BR_TIMER_REGISTER+4, value: brominatorTimeoutSP!)
