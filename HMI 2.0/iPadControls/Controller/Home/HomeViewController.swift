...
class ACTHomeViewController: UIViewController{
    ...
    func readWeplayData() -> [Int]{
        httpComm.httpGetResponseFromPath(url: "\(HTTP_PASS)\(SERVER_IP_ADDRESS):8080/\(READ_WEPLAY_SERVER_PATH)"){ (response) in
            guard let responseArray = response as? [Int] else { return }
            self.weplayData = responseArray
            //print("This is the read server path \(responseArray)")
        }
        return self.weplayData
    }
    
    func readFillerData() -> [Int]{
        httpComm.httpGetResponseFromPath(url: "\(HTTP_PASS)\(SERVER_IP_ADDRESS):8080/\(READ_FILLER_SERVER_PATH)"){ (response) in
            guard let responseArray = response as? [Int] else { return }
            self.fillerData = responseArray
            //print("This is the read server path \(responseArray)")
        }
    ...
    @IBAction func pushToStoryboard(_ sender: UIButton){
        ...
        if screenName != "" &&  screenName != "operationManual"{
            ...
            if screenName == "lights" {
                ...
                screen_Name = screenName
            } else if screenName == "supervisory" {
