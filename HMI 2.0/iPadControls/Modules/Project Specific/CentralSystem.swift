...
public class CentralSystem: NSObject, SimplePingDelegate{
    ...
    public func initialize(){
        ...
        timer = Timer.scheduledTimer(timeInterval: 0.6, target: self, selector: #selector(sendPing), userInfo: nil, repeats: true)
...
//                print("PLC CONNECTION FAILED: \(self.numberOfFailedPlcConnections). MAX AMOUNT OF FAIL REACHED")
    ...
    private func readBackWashRunning(){
        ...
        CENTRAL_SYSTEM?.readBits(length: 5, startingRegister: Int32(FILTRATION_BWASH_RUNNING_BIT), completion: { (success, response) in
            
            guard success == true else { return }
            
            let BW1status = Int(truncating: response![0] as! NSNumber)
            let BW2status = Int(truncating: response![2] as! NSNumber)
            let BW3status = Int(truncating: response![4] as! NSNumber)
